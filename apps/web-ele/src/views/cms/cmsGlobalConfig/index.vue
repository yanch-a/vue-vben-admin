...
<template>
  <Page auto-content-height :title="$tr('全局配置')">
    <template #extra>
      <ElButton type="primary" :loading="saving" @click="handleSave">{{ $tr('保存') }}</ElButton>
      <ElButton @click="fetchData">{{ $tr('刷新') }}</ElButton>
    </template>

    <ElForm v-loading="loading" label-width="140px" class="max-w-xl">
      <ElFormItem :label="$tr('项目路径')" required>
        <ElInput v-model="form.contextPath" maxlength="10" :placeholder="$tr('请输入项目路径')" />
      </ElFormItem>
      <ElFormItem :label="$tr('端口号')" required>
        <ElInputNumber v-model="form.sysPort" :controls="false" :max="999999" class="w-full" />
      </ElFormItem>
      <ElFormItem :label="$tr('系统所属地区编码')">
        <ElInput v-model="form.defaultCode" maxlength="50" :placeholder="$tr('请输入地区编码')" />
      </ElFormItem>
    </ElForm>
  </Page>
</template>
